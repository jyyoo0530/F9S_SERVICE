...
class DSBD_WKDETAIL @Inject()(cc: ControllerComponents) extends AbstractController(cc) {

  // Observer Pattern
  def responseAction(offernumber: String, baseyearweek: String): Action[AnyContent] = Action {

    val query = and(equal("offerNumber", offernumber), equal("baseYearWeek", baseyearweek))

    // Create Observable Object
    val observable: Observable[Document] =
      MongoConf.database
        .getCollection("F9S_DSBD_WKDETAIL")
        .find(query)
